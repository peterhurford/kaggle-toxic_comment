...
def add_features(train, test):
    ...
    print_step('BASIC FE 15/28')
    train2['chars_per_word'] = train2['num_chars'] / train2['num_words']
    test2['chars_per_word'] = test2['num_chars'] / test2['num_words']

    print_step('BASIC FE 16/28')
    train2['sentence'] = train2['comment_text'].apply(lambda x: [len(s) for s in re.split(r'[.!?\n]+', str(x))])
    test2['sentence'] = test2['comment_text'].apply(lambda x: [len(s) for s in re.split(r'[.!?\n]+', str(x))])

    print_step('BASIC FE 17/28')
    train2['num_sentence'] = train2['sentence'].apply(lambda x: len(x))
    test2['num_sentence'] = test2['sentence'].apply(lambda x: len(x))

    print_step('BASIC FE 18/28')
    train2['sentence_mean'] = train2.sentence.apply(lambda x: np.mean(x))
    test2['sentence_mean'] = test2.sentence.apply(lambda x: np.mean(x))

    print_step('BASIC FE 19/28')
    train2['sentence_max'] = train2.sentence.apply(lambda x: max(x) if len(x) > 0 else 0)
    test2['sentence_max'] = test2.sentence.apply(lambda x: max(x) if len(x) > 0 else 0)

    print_step('BASIC FE 20/28')
    train2['sentence_min'] = train2.sentence.apply(lambda x: min(x) if len(x) > 0 else 0)
    test2['sentence_min'] = test2.sentence.apply(lambda x: min(x) if len(x) > 0 else 0)

    print_step('BASIC FE 21/28')
    train2['sentence_std'] = train2.sentence.apply(lambda x: np.std(x))
    test2['sentence_std'] = test2.sentence.apply(lambda x: np.std(x))

    print_step('BASIC FE 22/28')
    ...
    test2['has_modal_then_verb'] = test2['pos_space'].apply(lambda ws: 'MD VB' in ' '.join([w[1] for w in ws]))
    ...
    toxic = ['crap', 'blowjob', 'blows me', 'blow me', 'go blow', 'butthole', 'buttwipe', 'dick',
            'dildo', r'\bf u\b', 'fart', 'gook', 'damn', r'goddam\b', 'f\*ck', 'f\*\*', 'di\*k',
    ...
    total = len(insult) + len(n_word) + len(mean_identity) + len(identity) + len(severe_toxic) + len(toxic) + \
            len(obscene) + len(wiki) + len(threat) + len(objects) + len(qualifiers) + len(polite) + 14
    ...
    count += 1; print_step('BAD WORDS ' + str(count) + '/' + str(total))
    train2['num_bad_words'] = train2[[c for c in train2.columns if 'has' in c and 'has_cat' not in c]].sum(axis=1) - train2[['has_' + str(i) for i in qualifiers] + ['has_' + str(i) for i in wiki] + ['has_' + str(i) for i in objects] + ['has_' + str(i) for i in polite]].sum(axis=1)
    test2['num_bad_words'] = test2[[c for c in test2.columns if 'has' in c and 'has_cat' not in c]].sum(axis=1) - test2[['has_' + str(i) for i in qualifiers] + ['has_' + str(i) for i in wiki] + ['has_' + str(i) for i in objects]].sum(axis=1)
