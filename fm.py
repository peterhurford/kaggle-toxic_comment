# Adapted from https://www.kaggle.com/shujian/stack-1-wordbatch-1-3-3-fm-ftrl-3/code, with further credit to antiip and Olivier
import re
import gc
import numpy as np
import pandas as pd
...
from preprocess import run_tfidf, clean_text, sigmoid, normalize_text
from cache import get_data, is_in_cache, load_cache, save_in_cache
from feature_engineering import add_features


# FM Model Definition
def runFM(train_X, train_y, test_X, test_y, test_X2, label, dev_index, val_index):
    class_weights = {'toxic': 1.0, 'severe_toxic': 0.2, 'obscene': 1.0,
                     'threat': 0.1, 'insult': 0.8, 'identity_hate': 0.2}
    model = FM_FTRL(alpha=0.02, beta=0.01, L1=0.00001, L2=30.0,
                    D=train_X.shape[1], alpha_fm=0.1,
    ...
    train_weight = np.array([1.0 if x == 1 else class_weights[label] for x in train_y])
...
def runChainedFM(train_X, train_y, test_X, test_y, test_X2, label, dev_index, val_index):
    print_step('Loading Lvl1')
    lvl1_train, lvl1_test = load_cache('lvl1_fm')
    [lvl1_train[c].apply(lambda x: 0 if x < 0.5 else 1) for c in lvl1_train.columns if 'fm_' in c and c != label]
    lvl1_train = csr_matrix(pd.concat([lvl1_train[c].apply(lambda x: 0 if x < 0.5 else 1) for c in lvl1_train.columns if 'fm_' in c and c != label], axis=1).values)
    lvl1_test = csr_matrix(pd.concat([lvl1_test[c].apply(lambda x: 0 if x < 0.5 else 1) for c in lvl1_test.columns if 'fm_' in c and c != label], axis=1).values)
...
submission['toxic'] = test['fm_toxic']
submission['severe_toxic'] = test['fm_severe_toxic']
submission['obscene'] = test['fm_obscene']
submission['threat'] = test['fm_threat']
submission['insult'] = test['fm_insult']
submission['identity_hate'] = test['fm_identity_hate']
submission.to_csv('submit/submit_lvl1_fm.csv', index=False)
print_step('Done')

print('~~~~~~~~~~~~~~~~~')
print_step('Run FM Lvl 2')
train, test = run_cv_model(label='lvl2_fm',
                           data_key='fm_data',
                           model_fn=runChainedFM,
                           train=train,
                           test=test,
                           kf=kf)
import pdb
...
save_in_cache('lvl2_fm', train, test)

print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
print_step('Prepping submission file')
submission = pd.DataFrame()
submission['id'] = test['id']
submission['toxic'] = test['lvl2_fm_toxic']
submission['severe_toxic'] = test['lvl2_fm_severe_toxic']
submission['obscene'] = test['lvl2_fm_obscene']
submission['threat'] = test['lvl2_fm_threat']
submission['insult'] = test['lvl2_fm_insult']
submission['identity_hate'] = test['lvl2_fm_identity_hate']
submission.to_csv('submit/submit_lvl2_fm.csv', index=False)
print_step('Done')
