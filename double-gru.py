# Following https://www.kaggle.com/zhbain/pooled-gru-fasttext-6c07c9/code
...
if not is_in_cache('lvl1_double-gru'):
    ...
    model.compile(loss='binary_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
    model.save_weights('cache/double-gru-model-weights.h5')
    ...
    for dev_index, val_index in kf.split(x_train, y_train[:, 0]):
        ...
        model.load_weights('cache/double-gru-model-weights.h5')
    ...
    for k, classx in enumerate(classes):
        train_df['double-gru_' + classx] = pred_train[:, k]
        test_df['double-gru_' + classx] = pred_full_test[:, k]

    print('~~~~~~~~~~~~~~~~~~')
    print_step('Cache Level 1')
    save_in_cache('lvl1_double-gru', train_df, test_df)
    print_step('Done!')
...
submission['toxic'] = test_df['double-gru_toxic']
submission['severe_toxic'] = test_df['double-gru_severe_toxic']
submission['obscene'] = test_df['double-gru_obscene']
submission['threat'] = test_df['double-gru_threat']
submission['insult'] = test_df['double-gru_insult']
submission['identity_hate'] = test_df['double-gru_identity_hate']
submission.to_csv('submit/submit_lvl1_double-gru.csv', index=False)
print_step('Done')
CV scores
# [('toxic', 0.9809688695646657),
#  ('severe_toxic', 0.9890132262588637),
#  ('obscene', 0.9901291537336672),
#  ('threat', 0.987693801067571),
#  ('insult', 0.9858315709601871),
#  ('identity_hate', 0.9851291861874717)]
# mean cv score : 0.9864609679620711
