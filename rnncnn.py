...
if not is_in_cache('lvl1_rnncnn'):
    ...
    y = Embedding(max_features, embed_size, weights=[embedding_matrix], trainable=False)(inp)
    ...
    model.compile(loss='binary_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
    model.save_weights('cache/rnncnn-model-weights.h5')
    ...
    for dev_index, val_index in kf.split(x_train, y_train[:, 0]):
        ...
        model.load_weights('cache/rnncnn-model-weights.h5')
