# Following https://www.kaggle.com/mosnoiion/two-rnn-cnn-columns-networks-with-keras?scriptVersionId=2745831/notebook
...
if not is_in_cache('lvl1_rnncnn2'):
    ...
    y = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)
    y = SpatialDropout1D(0.2)(x)
    y = Bidirectional(GRU(40, return_sequences=True, recurrent_dropout=0.1))(x)
    y = Conv1D(120, kernel_size=3, padding='valid', activation='relu', strides=1)(x)
    ...
    model.compile(loss='binary_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
    model.save_weights('cache/rnncnn2-model-weights.h5')
    ...
    for dev_index, val_index in kf.split(x_train, y_train[:, 0]):
        ...
        model.load_weights('cache/rnncnn2-model-weights.h5')
    ...
    for k, classx in enumerate(classes):
        train_df['rnncnn2_' + classx] = pred_train[:, k]
        test_df['rnncnn2_' + classx] = pred_full_test[:, k]

    print('~~~~~~~~~~~~~~~~~~')
    print_step('Cache Level 1')
    save_in_cache('lvl1_rnncnn2', train_df, test_df)
    print_step('Done!')
...
submission['toxic'] = test_df['rnncnn2_toxic']
submission['severe_toxic'] = test_df['rnncnn2_severe_toxic']
submission['obscene'] = test_df['rnncnn2_obscene']
submission['threat'] = test_df['rnncnn2_threat']
submission['insult'] = test_df['rnncnn2_insult']
submission['identity_hate'] = test_df['rnncnn2_identity_hate']
submission.to_csv('submit/submit_lvl1_rnncnn2.csv', index=False)
print_step('Done')
# [('toxic', 0.9817250985709102),
#          ('severe_toxic', 0.9902998981178206),
#           ('obscene', 0.9919374644196066),
#            ('threat', 0.9862254403199848),
#             ('insult', 0.9867898386844078),
#              ('identity_hate', 0.9863561129104594)]
# mean cv score : 0.9872223088371984
