...
from keras.layers import GRU, Bidirectional, GlobalMaxPooling1D, Reshape, Conv2D, Activation
from keras.layers import Dropout, Permute
...
if not is_in_cache('lvl1_gru-relu'):
    ...
    x = Conv2D(filters=64 , kernel_size=3, strides=2, padding='valid')(x)
    x = Activation('relu')(x)
    x = Dropout(0.2)(x)
    ...
    model.compile(loss='binary_crossentropy',
                  optimizer='adam',
                  metrics=['accuracy'])
    model.save_weights('cache/gru-relu-model-weights.h5')
    ...
    for dev_index, val_index in kf.split(x_train, y_train[:, 0]):
        ...
        model.load_weights('cache/gru-relu-model-weights.h5')
    ...
    for k, classx in enumerate(classes):
        train_df['gru-relu_' + classx] = pred_train[:, k]
        test_df['gru-relu_' + classx] = pred_full_test[:, k]

    print('~~~~~~~~~~~~~~~~~~')
    print_step('Cache Level 1')
    save_in_cache('lvl1_gru-relu', train_df, test_df)
    print_step('Done!')
...
submission['toxic'] = test_df['gru-relu_toxic']
submission['severe_toxic'] = test_df['gru-relu_severe_toxic']
submission['obscene'] = test_df['gru-relu_obscene']
submission['threat'] = test_df['gru-relu_threat']
submission['insult'] = test_df['gru-relu_insult']
submission['identity_hate'] = test_df['gru-relu_identity_hate']
submission.to_csv('submit/submit_lvl1_gru-relu.csv', index=False)
print_step('Done')
