...
if not is_in_cache('lvl1_gru'):
    ...
    for dev_index, val_index in kf.split(x_train, y_train[:, 0]):
        ...
        model.load_weights('cache/gru-model-weights.h5')
    ...
    save_in_cache('lvl1_gru', train_df, test_df)
    print_step('Done!')
...
submission['toxic'] = test_df['gru_toxic']
submission['severe_toxic'] = test_df['gru_severe_toxic']
submission['obscene'] = test_df['gru_obscene']
submission['threat'] = test_df['gru_threat']
submission['insult'] = test_df['gru_insult']
submission['identity_hate'] = test_df['gru_identity_hate']
submission.to_csv('submit/submit_lvl1_gru.csv', index=False)
print_step('Done')
